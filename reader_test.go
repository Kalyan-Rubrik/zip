...
func readTestZipIter(t *testing.T, zt ZipTest) {
	var err error
	var raw []byte
	path := filepath.Join("testdata", zt.Name)
	if zt.Obscured {
	...
	if err != nil {
		t.Errorf("unable to open file: %s, %v", path, err)
	}
	zipStat, err := zipFile.Stat()
	if err != nil {
		t.Errorf("unable to stat file: %s, %v", path, err)
	}
	var rc *Iterator
	rc, err = NewIterator(zipFile, zipStat.Size())
	if err != nil {
		t.Errorf("unable to create iterator for: %s, %v", path, err)
	}
	...
	if rc.Comment != zt.Comment {
		t.Errorf("comment=%q, want %q", rc.Comment, zt.Comment)
	}
	...
	for af, err = rc.Next(); !HasNoMoreEntries(err); af, err = rc.Next() {
		if err != nil {
			t.Errorf("unexpected error during iteration: %v", err)
		}
		actFiles = append(actFiles, af)
	}

	if err != done && err != zt.Error {
		t.Errorf("error=%v, want %v", err, zt.Error)
		return
	}
	...
	// test read of each file
	for i, ft := range zt.File {
		readTestFile(t, zt, ft, actFiles[i], raw)
	}
	if t.Failed() {
		return
	}

	// reset iterator and repeat the iteration
	err = rc.Reset()
	if err != nil {
		t.Errorf("error resetting the iterator: %v", err)
	}

	actFiles = make([]*File, 0, len(actFiles))
	for af, err = rc.Next(); !HasNoMoreEntries(err); af, err = rc.Next() {
		if err != nil {
			t.Errorf("unexpected error during iteration: %v", err)
		}
	...
	for i := 0; i < 5; i++ {
		for j, ft := range zt.File {
			go func(j int, ft ZipTestFile) {
				readTestFile(t, zt, ft, actFiles[j], raw)
				done <- true
			}(j, ft)
			n++
		}
